...
namespace RDA.Data {
  ...
  public class Asset {
    ...
    private void ProcessElement_Standard(XElement element) {
      ...
      if (element.Element("WikiPage")?.Value is string wiki) {
        this.Wiki = "https://wiki.related-designs.de/wiki/display/Anno5/" + wiki;
      }
    ...
    private void ProcessElement_Item(XElement element) {
      ...
      if (element.Element("ItemTypeText")?.Value is string itemtype) {
        ...
        this.IsScienceItem = element.Element("IsScienceItem")?.Value == "1";
      }

      //this.ItemCategory = element.Element("ItemCategory")?.Value;
      this.ItemTarget = element.Element("ItemTarget")?.Value;
      if (element.Element("ItemHonourPrice")?.Value is var str) {
      ...
      if (element.Element("ItemFaction")?.Value is string str2)
        this.ItemFaction = new Description(str2);

      if (element.Element("ItemAllocationCategory")?.Value is string str3)
        this.ItemAllocationCategory = new Description(Engine.IdToCategory[str3]);
      if (element.Element("ItemDescriptionText")?.Value is string str4)
        this.ItemDescription = new Description(str4);
      if (element.Element("ItemBuffIconDescriptionText")?.Value is string str5)
        this.BuffDescription = new Description(str5);
    }
